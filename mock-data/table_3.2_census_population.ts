 ...
 * 
 * TypeScript interface and data for frontend/API usage
 */

export interface CensusPopulationRecord {
  year: number;
  total_population: number;
  variation_in_population: number | null;
  decennial_percentage_increase: number | null;
...
export const censusPopulationData: CensusPopulationRecord[] = [
  {
    year: 1901,
...
export const getCensusDataByYear = (year: number): CensusPopulationRecord | undefined => {
  return censusPopulationData.find(record => record.year === year);
};

export const getCensusDataRange = (startYear: number, endYear: number): CensusPopulationRecord[] => {
